...
public class MessageDB {
    ...
    public void saveMsg(String id, Messagex msg) {
        db.execSQL("CREATE table IF NOT EXISTS _"
                + id
                + " (_id INTEGER PRIMARY KEY AUTOINCREMENT,conversationId TEXT, content TEXT,from TEXT,timestamp TEXT,messageId TEXT,type TEXT)");
        //int isCome = 0;
...
//		}
        db.execSQL(
                "insert into _"
                        + id
                        + " (conversationId,content,from,timestamp,messageId,type) values(?,?,?,?,?,?)",
                new Object[]{msg.getConversationId(), msg.getContent(), msg.getFrom(), msg.getTimestamp(), msg.getMessageId(), msg.getMessagetype()});
    }

    public List<Messagex> getMsg(String id, int pager) {
        List<Messagex> list = new LinkedList<Messagex>();
        int num = 10 * (pager + 1);// 做滚动到顶端自动加载数据
        db.execSQL("CREATE table IF NOT EXISTS _"
                + id
                + " (_id INTEGER PRIMARY KEY AUTOINCREMENT,conversationId TEXT, content TEXT,from TEXT,timestamp TEXT,messageId TEXT,type TEXT)");
